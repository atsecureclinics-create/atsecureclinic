...
const data = [
  ...
  },
  {
    ...
    solution: "Prevention comes first. Prevent before a problem arises. Prevention focussed solutions.",
  },
];
      ...
      <div>
        {visible.map((item, index) => (
          ...
          >
            ...
            <div
              className={`font-opensans px-3 sm:px-5 md:px-8 py-3 sm:py-4 md:py-5 bg-background ${
                index === visible.length - 1 && expanded ? "rounded-br-2xl" : ""
              }`}
            >
              <p className="text-sm sm:text-base md:text-xl text-navy leading-relaxed max-w-xl">
                {item.solution}
      ...
      {data.length > INITIAL_COUNT && (
        <div className="flex justify-center mt-6 md:mt-8">
          <button
            ...
            className="font-opensans font-semibold text-sm md:text-base text-navy border border-navy rounded-full px-8 py-3 hover:bg-navy hover:text-white transition-all duration-300"
          >
            {expanded ? "Show Less" : `Read More (${data.length - INITIAL_COUNT} more)`}
          </button>
        </div>
      )}
    </section>
